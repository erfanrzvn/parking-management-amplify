...
exports.handler = async (event) => {
  requireAdmin(event);
  if ('householdId' in event.arguments.input || 'residentCode' in event.arguments.input) throw new Error('Resident codes are generated by the system');
  ...
  try {
    ...
    if (!currentResident.Item || currentResident.Item.deletedAt) {
      throw new Error(`Resident with ID ${id} not found`);
    }
    
    const userId = currentResident.Item.cognitoUsername || currentResident.Item.userId;
    const merged = { ...currentResident.Item, ...event.arguments.input };
    for (const field of ["building", "floor", "unitNumber", "plate"]) {
      if (!String(merged[field] || "").trim()) throw new Error(`${field} is required`);
    }
    if (email !== undefined && email !== currentResident.Item.email) throw new Error("Email changes require an account migration");
    const householdId = await resolveHouseholdCode(docClient, merged);
    
    // Step 2: Build DynamoDB update expression
    const now = new Date().toISOString();
    const updateExpressions = ['householdId = :householdId', 'residentCode = :householdId'];
    const expressionAttributeNames = { '#updatedAt': 'updatedAt' };
    const expressionAttributeValues = { ':updatedAt': now, ':householdId': householdId };
