...
interface Resident {
  ...
  plate?: string;
  userId: string;
  householdId: string; // Required - shared by all household members
}

interface Reservation {
  id: string;
  residentId: string;
  householdId: string; // Household code instead of residentCode
...
export default function AdminPanel({ user }: AdminPanelProps) {
  ...
  const getResidentInfo = (residentId: string) => {
    ...
    if (reservation && reservation.householdId) {
      return {
        name: reservation.residentFloor 
          ? `Floor ${reservation.residentFloor}` 
          : 'Resident',
        code: reservation.householdId,
        floor: reservation.residentFloor,
        building: undefined,
