...
async function main() {
  ...
  if (result.data?.listResidents?.items) {
    const residents = result.data.listResidents.items;
    console.log(`Found ${residents.length} residents:\n`);
    residents.forEach((r, i) => {
      console.log(`${i + 1}. ${r.name}`);
      console.log(`   ID: ${r.id}`);
      console.log(`   Code: ${r.residentCode}`);
      console.log(`   Floor: ${r.floor} | Plate: ${r.plate}`);
      console.log('');
    });
  } else {
