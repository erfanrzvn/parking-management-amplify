...
async function main() {
  // Check CreateReservationInput type fields
  const query = `
    {
      __type(name: "CreateReservationInput") {
        name
        inputFields {
          name
          type {
  ...
  console.log('🔍 Checking CreateReservationInput schema...\n');
  const result = await makeRequest(query);
  
  if (result.data?.__type) {
    console.log('✅ CreateReservationInput fields:');
    result.data.__type.inputFields.forEach(field => {
